...
const closeDrawerBtn = document.getElementById('close-drawer'); // Added reference for close button
...
const renderOverUnder = () => {
    ...
    if (gamesData && gamesData.overUnder) {
...
const renderTopSecret = () => {
    ...
    if (gamesData && gamesData.topSecret) {
...
const renderUltimate = () => {
    ...
    if (gamesData && gamesData.ultimate) {
...
// 8. PRIVACY POLICY VIEW (NEW)
const renderPrivacyPolicy = () => {
    headerTitle.innerText = "PRIVACY POLICY";
    return `
...
// --- Populate Navigation Drawer Function (NEW) ---
...
const navigate = (view) => {
    ...
    switch(view) {
        ...
        case 'privacy-policy': contentDiv.innerHTML = renderPrivacyPolicy(); break; // New Case
        default: contentDiv.innerHTML = renderHome();
    }
};

// Initialize
populateNavigationDrawer(); // Populate drawer on load
navigate('home');

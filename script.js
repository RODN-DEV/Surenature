...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const navigate = (view, addToHistory = true) => {
        ...
        if (vipViews.includes(view) && !isAuthenticated) {
            if (passwordModal) {
                ...
                // Save the view we wanted to go to, so we can go there after password
                passwordModal.dataset.targetView = view; 
            }
            return;
        }

        // Update History (This fixes the back button issue)
        if (addToHistory) {
    ...
    window.addEventListener('popstate', (event) => {
        if (event.state && event.state.view) {
            // Go to the view from history, but DON'T push to history again
            navigate(event.state.view, false);
        } else {
            // If we run out of history, go home
            navigate('home', false);
        }
    });
    ...
    const populateNavigationDrawer = () => {
        ...
        if (!navLinksContainer) {
            console.error("Navigation container 'nav-links' not found in HTML.");
            return;
        }
    ...
    if (submitPasswordBtn && passwordInput) {
        submitPasswordBtn.addEventListener('click', () => {
            ...
            if (validPasswords.includes(password)) {
                ...
                // Navigate to the pending view if set
    ...
    // 6. View Templates (Render Functions)
    
    // HOME
    const renderHome = () => {
        if (headerTitle) headerTitle.innerText = "FOOTBALL SIMPLE";
        // Need to attach global onclick handlers for these buttons since they are injected as strings
    ...
    // Check hash for initial view (e.g., if user refreshes on #free-tips)
    const initialView = window.location.hash.substring(1) || 'home';
    
    // Set initial history state without triggering animation loop
    history.replaceState({ view: initialView }, '', `#${initialView}`);
    
    // Render initial view
    navigate(initialView, false); // false means don't push state again
});
